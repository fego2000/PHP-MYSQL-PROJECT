...
$all_team = $conn->query($sql);
$all = array();

foreach ($all_team as $key => $value) {
	$id = $value['id'];
$sql1 = "select social_media.name,link from team_social,social_media where team_social.team_id = $id and team_social.social_id = social_media.id ";

$details = $conn->query($sql1);

$all[] = array('id2' =>$value['id'],'name2' =>$value['name'], 'alldetail' =>$details); 	
}
...
?>
		...
		<?php foreach ($all as $key => $value) { ?>
				   <tr>
				      <td scope="row"><?php echo $value['id2']; ?></td>		
				      <td><?php echo $value['name2']; ?></td>     
				      <td>
				      	<ul>
				      	<?php foreach ($value['alldetail'] as $key => $value2) { ?> 	
				      	   <li><a href="<?php echo $value2['link']; ?>"><?php echo $value2['link']; ?></a></li>	
				      	<?php } ?>   
				      	</ul>
				      </td>
				      <td>
